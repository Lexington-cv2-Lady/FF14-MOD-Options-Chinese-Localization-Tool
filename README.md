...
5. **翻译**：点「AI 翻译」调用 AI 批量翻译（每批发给 AI 翻译的英文条数可在「每批条数」中调整）；翻译结果可逐条人工校正（对照词典辅助）。若已用其它工具或网页 AI 翻译完成，也可通过「已翻译的文件，写入词典」把结果合并进词典后再应用。
6. **应用翻译**：确认无误后点「3. 词典写入Mod」，把词典里的汉化文本写回 MOD。程序会在应用前自动备份，应用后按 F4「恢复备份」可撤销。若某条 AI 翻译不理想，可在词典目录的「个性翻译.json」里按「英文:中文」单独指定译法，应用时会自动覆盖对应词条（命中黑名单的词条不生效）。
